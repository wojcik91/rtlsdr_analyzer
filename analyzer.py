...
class Analyzer(QtGui.QMainWindow):
    ...
    def createPlot(self):
        ...
        self.mouseProxy = pg.SignalProxy(
            self.plot.scene().sigMouseMoved, rateLimit=20, slot=self.mouseMoved
        )
    ...
    def updateFreqs(self):
        self.freqs = np.arange(
            self.startFreq + self.step / 2, self.stopFreq + self.step / 2, self.step
        )
        self.markerIndex = [None, None, None, None]
        self.deltaIndex = None
        self.peakIndex = None
        ...
        if len(self.xData) == self.sliceLength * len(self.freqs):
            if self.AVERAGE:
                if self.avgCounter == 0:
                    if self.avgArray is None:
                        ...
                        self.avgArray = np.append(
                            self.avgArray, np.array([self.yData]), axis=0
                        )
            ...
            for i in range(len(self.MARKERS)):
                if self.MARKERS[i]:
                    ...
                    self.markerText[i].setText(
                        "Mk1:\nf=%0.1f MHz\nP=%0.1f dBm"
                        % (self.xData[self.markerIndex[i]], yData[self.markerIndex[i]])
                    )
            ...
            if self.PEAK:
                ...
                self.peakText.setText(
                    "Peak:\nf=%0.1f MHz\nP=%0.1f dBm"
                    % (self.xData[self.peakIndex], yData[self.peakIndex])
                )
    ...
    def waterfallUpdate(self, xData, yData):
        ...
        self.waterfallImg.setImage(
            self.waterfallImgArray.T, autoLevels=True, autoRange=False
        )
    ...
    def setupWorker(self):
        ...
        self.worker = Worker(
            self.nfft, self.length, self.sliceLength, self.sampRate, self.nwelch
        )
    ...
    def mouseMoved(self, evt):
        ...
        if self.plot.sceneBoundingRect().contains(pos):
            ...
            self.posLabel.setText(
                "f=%0.1f MHz, P=%0.1f dBm" % (mousePoint.x(), mousePoint.y())
            )
